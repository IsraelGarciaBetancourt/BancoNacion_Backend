...
def crear_solicitud(
    ...
    """
    tea = Decimal('0.1299') if con_seguro else Decimal('0.1799')
    ...
    tem_int = Decimal(math.pow(1 + float(tea), 1/12.0) - 1)
    tasa_desg = Decimal('0.0012') if con_seguro else Decimal('0')
    tem = tem_int + tasa_desg
    if tem == 0:
...
def evaluar_solicitud(conn: Connection, pksolicitud: int) -> dict:
    ...
    tea = sol["tasainterescompensatoria"] or Decimal('0.1799')
    ...
    # Determinamos si tiene seguro de desgravamen según la tasa (12.99% con seguro, 17.99% sin seguro)
    # Las tasas antiguas eran 40.92% con seguro y 43.92% sin seguro.
    has_insurance = (tea == Decimal('0.1299') or tea == Decimal('0.4092'))
    tasa_desg = Decimal('0.0012') if has_insurance else Decimal('0')
